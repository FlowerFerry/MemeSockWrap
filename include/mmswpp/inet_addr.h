...
struct inet_addr
{
    ...
    using address_storage = std::variant<
        std::monostate, // Invalid address
    ...
    bool operator==(const inet_addr& _other) const
    {
        if (storage() == _other.storage())
        {
            ...
            case storage_type::inet4:
                return std::get<in4_addr>(addr_).s_addr == std::get<in4_addr>(_other.addr_).s_addr;
            case storage_type::inet6:
                return memcmp(std::get<in6_addr>(addr_).s6_addr, std::get<in6_addr>(_other.addr_).s6_addr, 16) == 0;
            case storage_type::string:
                return string_equal(
                    std::get<memepp::string>(addr_), family_, std::get<memepp::string>(_other.addr_), _other.family_);
            default:
        ...
        case storage_type::inet4:
        {
            ...
            return std::get<in4_addr>(addr_).s_addr == addr4.s_addr;
        } break;
        case storage_type::inet6:
        {
            ...
            return memcmp(std::get<in6_addr>(addr_).s6_addr, addr6.s6_addr, 16) == 0;
        } break;
        case storage_type::string:
        {
            auto str = _other.to_string();
            return string_equal(
                std::get<memepp::string>(addr_), family_, str, _other.family_);
        } break;
        default:
    ...
    std::tuple<in4_addr, bool> to_in4_addr() const noexcept
    {
        if (is_ipv4()) {
            if (storage() == storage_type::inet4) {
                return { std::get<in4_addr>(addr_), true };
            }
            if (storage() == storage_type::string) {
                in4_addr addr;
                if (inet_pton(AF_INET, std::get<memepp::string>(addr_).c_str(), &addr) == 1) 
                    return { addr, true };
            }
        }
        ...
        if (is_ipv4_mapped()) {
            if (storage() == storage_type::inet6) {
                in4_addr addr;
                std::memcpy(&addr, &(std::get<in6_addr>(addr_).s6_addr[12]), sizeof(in4_addr));
                return { addr, true };
            }

            if (storage() == storage_type::string) {
                in6_addr addr6;
                if (inet_pton(AF_INET6, std::get<memepp::string>(addr_).c_str(), &addr6) == 1) 
                {
                    in4_addr addr4;
    ...
    std::tuple<in6_addr, bool> to_in6_addr() const noexcept
    {
        if (is_ipv6()) {
            if (storage() == storage_type::inet6) {
                return { std::get<in6_addr>(addr_), true };
            }
            if (storage() == storage_type::string) {
                in6_addr addr;
                if (inet_pton(AF_INET6, std::get<memepp::string>(addr_).c_str(), &addr) == 1) 
                    return { addr, true };
            }
        }
    ...
    memepp::string to_string() const
    {
        ...
        case storage_type::inet4:
        {
            ...
            if (inet_ntop(AF_INET, &std::get<in4_addr>(addr_), buffer, INET_ADDRSTRLEN) == nullptr)
            {
        ...
        case storage_type::inet6:
        {
            ...
            if (inet_ntop(AF_INET6, &std::get<in6_addr>(addr_), buffer, INET6_ADDRSTRLEN) == nullptr)
            {
                return memepp::string{};
            }
            return memepp::string{ buffer };
        ...
        case storage_type::string:
        {
            return std::get<memepp::string>(addr_);
        }
        default: {
            return memepp::string{};
        }
    ...
    {
        return std::get_if<in4_addr>(&addr_);
    }

    const in6_addr* get_in6_addr_ptr() const noexcept
    {
        return std::get_if<in6_addr>(&addr_);
    }

    const memepp::string* get_str_ptr() const noexcept
    {
        return std::get_if<memepp::string>(&addr_);
    }
    ...
    void assign(const memepp::string& _str)
    {
        ...
        family_ = identify_family(std::get<memepp::string>(addr_));
    }
    ...
    bool convert(storage_type _to_storage)
    {
        ...
        case storage_type::inet6:
        {
            if (storage() == storage_type::inet4)
            {
                addr_   = make_ipv4_mapped(std::get<in4_addr>(addr_));
                family_ = family_type::ipv4_mapped;
                return true;
            }
            ...
            if (storage() == storage_type::string)
            {
                ...
                {
                    in4_addr addr4;
                    if (inet_pton(AF_INET, std::get<memepp::string>(addr_).c_str(), &addr4) != 1)
                    {
                        return false;
                    }
    ...
    inline void split_domain(std::back_insert_iterator<_Container> _out) const
    {
        ...
        auto& str = std::get<memepp::string>(addr_);
        str.split(".", _out);
    }
